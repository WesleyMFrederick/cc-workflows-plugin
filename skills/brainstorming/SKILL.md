...
- Must include:
  ...
  - **Artifacts & Paths** — every file reviewed during recon. Use markdown links for `.md` files and codeblock paths for all other file types. **All local file paths must be full absolute paths** — never partial filenames or relative paths.
  - **Recon Findings** — synthesized understanding, measurements, key data structures
  - **Open questions** — place inline in the relevant bucket (Baseline, Ideal, or Delta), not in a standalone section at the end. A top-level Open Questions section is only for truly cross-cutting unknowns that don't belong in any bucket.
- **Write the whiteboard BEFORE asking the user any Phase 1 questions.** Do not gate the whiteboard write on receiving answers. Research → write → then ask.
...
- **Gathering mode:** In Phase 1, prefer open-ended questions or additional research over `AskUserQuestion` with structured choices. If the user signals "gathering mode" (e.g., "find evidence", "research this", "idk — figure it out"), answer with observations, measurements, and inferences — NOT decision prompts. Structured choices belong in Phase 2.
- **Use `AskUserQuestion` tool** only when you need the human to make a judgement call among real, ready-to-decide alternatives. If you're still gathering evidence, the alternatives aren't ready yet — don't present them.
