...
1. **Create worktree in sibling directory**
   ...
   > **IMPORTANT:** Run these as **separate Bash calls** — do NOT chain with `&&` or combine in one call.

   Call 1 — get parent directory:
   ```bash
   dirname "$(git rev-parse --show-toplevel)"
   ```

   Call 2 — create worktree using the resolved path from Call 1:
   ```bash
   git worktree add "/resolved/parent/$worktree_dir" -b "$branch_name"
   ```
